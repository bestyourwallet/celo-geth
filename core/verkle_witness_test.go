...
import (
	...
	"github.com/bestyourwallet/celo-geth/trie/utils"
...
func TestProcessVerkle(t *testing.T) {
	var (
		...
		intrinsicContractCreationGas, _ = IntrinsicGas(code, nil, nil, true, true, true, true, nil, nil)
		...
		intrinsicCodeWithExtCodeCopyGas, _ = IntrinsicGas(codeWithExtCodeCopy, nil, nil, true, true, true, true, nil, nil)
...
func TestProcessParentBlockHash(t *testing.T) {
	...
	checkBlockHashes := func(statedb *state.StateDB, isVerkle bool) {
		...
		for i := 1; i <= num; i++ {
			...
			vmContext := NewEVMBlockContext(header, nil, new(common.Address), chainConfig, statedb)
